...
def trainValidateSegmentation(args):
    ...
    elif args.file_root == 'GZCD':
        args.file_root = 'D:\yanjiu\L-Net_L\GZCD'
    else:
    ...
    # input_tensor1 = torch.randn(1, 3, args.inHeight, args.inWidth).cuda()  # Create a dummy input tensor x1
    # input_tensor2 = torch.randn(1, 3, args.inHeight, args.inWidth).cuda()  # Create a dummy input tensor x2
    # flops = FlopCountAnalysis(model, (input_tensor1, input_tensor2))  # Compute FLOPs with both inputs
    # # Convert FLOPs to G (Giga FLOPs)
    # flops_in_G = flops.total() / 1e9  # Convert from FLOPs to G
    # print(f"FLOPs: {flops_in_G:.2f} G")  # Print FLOPs in G (Gigaflops)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--file_root', default="LIVER", help='Data directory | LIVER | GZCD | SYSU |CDTH')
    parser.add_argument('--inWidth', type=int, default=256, help='Width of RGB image')
    parser.add_argument('--inHeight', type=int, default=256, help='Height of RGB image')
    parser.add_argument('--max_steps', type=int, default=40000, help='Max. number of iterations')  #####
    parser.add_argument('--num_workers', type=int, default=12, help='No. of parallel threads')   #4
    parser.add_argument('--batch_size', type=int, default=32, help='Batch size')  #32   16
    parser.add_argument('--step_loss', type=int, default=100, help='Decrease learning rate after how many epochs')
    parser.add_argument('--lr', type=float, default=1e-4, help='Initial learning rate')  #1e-4,5e-4
